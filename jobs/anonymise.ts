...
/**
 * Turns a restored production dump into the anonymised copy that preview environments
 * and developer machines run on. Production runs this nightly against a scratch database
 * (see `jobs/backup.ts`), so raw personal data never leaves the production environment.
 */

export const ANONYMISED_DB_NAME = 'catalyse_anon'

/**
 * The scratch database on the same server as `dbUrl`, which must not itself be the
 * scratch database: the restore empties whatever it points at.
 */
export function anonymisedDbUrl(dbUrl: string): string {
  const url = new URL(dbUrl)
  if (url.pathname.slice(1) === ANONYMISED_DB_NAME)
    throw new Error(`DATABASE_URL already points at ${ANONYMISED_DB_NAME}`)
  url.pathname = `/${ANONYMISED_DB_NAME}`
  return url.toString()
}
...
/** Restores `dumpPath` into `dbUrl`, anonymises it and adds the dev accounts. */
export async function buildAnonymisedCopy(dumpPath: string, dbUrl: string): Promise<void> {
