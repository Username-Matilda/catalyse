...
test.describe('Project Discovery', () => {
  test('Volunteer searches projects by keyword', async ({ adminPage, volunteer }) => {
    const keyword = `E2ESearch${Date.now()}`;
    const title = `${keyword} Project`;
    await adminCreateProject(adminPage, title, 'A searchable project for discovery tests');
  ...
  test('Volunteer filters projects by "Seeking Help" status', async ({ adminPage, volunteer }) => {
    const title = `E2E Discovery Seeking ${Date.now()}`;
    // Admin-created projects have is_seeking_help = true by default
    await adminCreateProject(adminPage, title, 'Project for seeking-filter discovery test');

    await volunteer.page.goto(`${BASE_URL}/static/index.html`);
    await expect(volunteer.page.getByRole('heading', { name: 'Projects', exact: true })).toBeVisible({ timeout: 10_000 });

    // Open the Needs filter and select "Seeking Help" (client-side filter)
