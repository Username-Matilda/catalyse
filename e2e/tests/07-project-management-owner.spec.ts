...
async function setupOwnedProject(adminPage: Page, volunteer: { page: Page; name: string }): Promise<number> {
  const title = `E2E Owner ${Date.now()}`;
  const projectId = await proposeProject(volunteer.page, title, 'Setup description for owner management');
...
test.describe('Project Management (Owner)', () => {
  test('Project owner edits project details', async ({ adminPage, volunteer }) => {
    const projectId = await setupOwnedProject(adminPage, volunteer);
    ...
    await volunteer.page.waitForURL(`${BASE_URL}/static/project.html?id=${projectId}`, { timeout: 15_000 });
  ...
  test('Project owner adds a required skill to the project', async ({ adminPage, volunteer }) => {
    const projectId = await setupOwnedProject(adminPage, volunteer);
  ...
  test('Project owner posts a progress update', async ({ adminPage, volunteer }) => {
    const projectId = await setupOwnedProject(adminPage, volunteer);
