...
import { CreateProjectUpdateSchema } from '@/lib/schemas'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = parseBody(CreateProjectUpdateSchema, raw)
  if (!parsed.success) return parsed.response

  const update = await prisma.projectUpdate.create({
    data: { projectId, authorId: volunteer.id, content: parsed.data.content },
  })
