    ...
    <>
      ...
      <main className="max-w-350 w-full mx-auto px-6 py-5 pb-15">
        ...
        <div className="mb-6" style={{ maxWidth: 240 }}>
          <label htmlFor="status-filter" className="text-sm text-text-light block mb-1">Status</label>
        ...
        {loadingData ? (
          ...
          <div className="bg-surface rounded-xl shadow p-6 text-center">
            ...
            <p className="text-text-light">Create one to verify a volunteer&apos;s skills before giving them a bigger project.</p>
          </div>
        ) : (
          ...
          tasks.map(task => {
            ...
            return (
              <div key={task.id} className="card bg-surface rounded-xl shadow p-6 mb-4 overflow-hidden wrap-break-word">
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div className="text-text-light" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: '0.8rem' }}>
                        ...
                        {task.assigned_to_id && task.assigned_to_name && (
                          <span>Assigned to: <Link href={`/admin/volunteers/${task.assigned_to_id}`} onClick={e => e.stopPropagation()}>{task.assigned_to_name}</Link></span>
                        )}
                        {task.review_rating && (
                          <span style={{ color: RATING_COLORS[task.review_rating] }}>{RATING_LABELS[task.review_rating]}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  {/* [test hook] status-badge class used as test selector */}
                  <span className="status-badge" style={{ padding: '2px 8px', borderRadius: 12, fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap', flexShrink: 0, ...STATUS_STYLES[task.status] }}>
                    {task.status}
                  </span>
                </div>
                ...
                {expanded && (
                  <>
                    <p style={{ whiteSpace: 'pre-wrap', margin: '0 0 12px', fontSize: '0.9rem' }}>{task.description}</p>

                    {task.review_rating && (
                      <p style={{ marginBottom: 8, fontWeight: 500, color: RATING_COLORS[task.review_rating] }}>
                        Rating: {RATING_LABELS[task.review_rating]}
                      </p>
                    )}
                    {task.review_notes && (
                      <p style={{ fontSize: '0.875rem', color: 'var(--text-light)', marginBottom: 8 }}>Notes: {task.review_notes}</p>
                    )}
                    {task.feedback_to_volunteer && (
                      <div style={{ background: 'var(--accent)', borderRadius: 'var(--radius)', padding: '10px 14px', marginBottom: 12, fontSize: '0.875rem' }}>
                        <strong>Feedback to volunteer:</strong> {task.feedback_to_volunteer}
                      </div>
                    )}

                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                      <span style={{ fontSize: '0.875rem', color: 'var(--text-light)' }}>Created {formatDate(task.created_at)}</span>
                      <div className="flex gap-2">
                        <Button variant="secondary" size="sm" onClick={e => { e.stopPropagation(); openEdit(task) }}>Edit</Button>
                        {task.status === 'open' && (
                          <Button variant="secondary" size="sm" onClick={e => { e.stopPropagation(); setAssignModal(task); setAssignVolunteerId('') }}>Assign</Button>
                        )}
                        {task.assigned_to_id && task.status !== 'open' && task.status !== 'completed' && task.status !== 'reviewed' && (
                          <Button variant="secondary" size="sm" onClick={e => { e.stopPropagation(); unassignTask(task.id) }}>Unassign</Button>
                        )}
                        {task.status === 'submitted' && (
                          <Button size="sm" onClick={e => { e.stopPropagation(); setReviewModal(task); setReviewRating('good'); setReviewFeedback(''); setReviewNotes('') }}>Review</Button>
                        )}
                      </div>
                    </div>
                  </>
      ...
      {showCreate && (
        ...
        >
          <div role="dialog" aria-labelledby="create-dialog-title" className="bg-surface rounded-xl shadow-lg max-w-150 w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <form onSubmit={createTask}>
                <div className="mb-5">
                  ...
                  <input id="ct-title" type="text" value={createTitle} onChange={e => setCreateTitle(e.target.value)} required autoFocus placeholder="e.g., Draft 3 social media posts about AI safety" />
                </div>
                <div className="mb-5">
                  <label htmlFor="ct-desc">Description</label>
                  <textarea id="ct-desc" rows={4} value={createDesc} onChange={e => setCreateDesc(e.target.value)} required placeholder="What should the volunteer do? Include any context, examples, or guidelines." />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="mb-5">
                  <div>
                    <label htmlFor="ct-skill">Skill Being Tested</label>
                    <select id="ct-skill" value={createSkillId} onChange={e => setCreateSkillId(e.target.value)}>
                  ...
                  <div>
                    ...
                    <input id="ct-hours" type="number" min="0.5" max="20" step="0.5" value={createHours} onChange={e => setCreateHours(e.target.value)} placeholder="e.g., 2" />
                  </div>
                </div>
      ...
      {editModal && (
        ...
        >
          <div role="dialog" aria-labelledby="edit-dialog-title" className="bg-surface rounded-xl shadow-lg max-w-150 w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-5 border-b border-brand-border flex justify-between items-center">
            ...
            <div className="p-6">
              <form onSubmit={editTask}>
                <div className="mb-5">
                  ...
                  <input id="et-title" type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} required autoFocus />
                </div>
                <div className="mb-5">
                  <label htmlFor="et-desc">Description</label>
                  <textarea id="et-desc" rows={4} value={editDesc} onChange={e => setEditDesc(e.target.value)} required />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="mb-5">
                  <div>
                    <label htmlFor="et-skill">Skill Being Tested</label>
                    <select id="et-skill" value={editSkillId} onChange={e => setEditSkillId(e.target.value)}>
                      <option value="">None specific</option>
                      {skills.map(s => (
                        <option key={s.id} value={s.id}>{s.name} ({s.category_name})</option>
                      ))}
                  ...
                  <div>
                    ...
                    <input id="et-hours" type="number" min="0.5" max="20" step="0.5" value={editHours} onChange={e => setEditHours(e.target.value)} />
                  </div>
                </div>
                <div className="px-0 py-4 border-t border-brand-border flex gap-3 justify-end">
            ...
            <div className="p-6">
              ...
              <form onSubmit={assignTask}>
                <div className="mb-5">
                  ...
                  <select id="assign-vol" value={assignVolunteerId} onChange={e => setAssignVolunteerId(e.target.value)} required>
      ...
      {reviewModal && (
        ...
        >
          <div role="dialog" aria-labelledby="review-dialog-title" className="bg-surface rounded-xl shadow-lg max-w-150 w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-5 border-b border-brand-border flex justify-between items-center">
            ...
            <div className="p-6">
              ...
              <form onSubmit={reviewTask}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                    {(['excellent', 'good', 'needs_improvement'] as const).map(r => (
                      <label key={r} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                        ...
                        <span><strong>{RATING_LABELS[r]}</strong>{r === 'excellent' ? ' — Exceeded expectations' : r === 'good' ? ' — Met expectations' : ' — Not quite there yet'}</span>
                      </label>
                    ))}
                  </div>
                </div>
                <div className="mb-5">
                  ...
                  <textarea id="rv-notes" rows={2} value={reviewNotes} onChange={e => setReviewNotes(e.target.value)} placeholder="Your assessment…" />
                </div>
                <div className="mb-5">
                  <label htmlFor="rv-feedback">{"Feedback to Volunteer (they'll see this)"}</label>
                  <textarea id="rv-feedback" rows={3} value={reviewFeedback} onChange={e => setReviewFeedback(e.target.value)} placeholder="Constructive feedback…" />
                </div>
                <div className="px-0 py-4 border-t border-brand-border flex gap-3 justify-end">
