...
const ICONS: Record<ToastType, React.ReactNode> = {
  ...
  success: (
    <svg className="shrink-0 text-green-500" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/>
    </svg>
  ),
  info: (
    <svg className="shrink-0 text-blue-500" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex flex-col gap-2.5 w-[360px] pointer-events-none max-sm:w-[calc(100vw-24px)] max-sm:bottom-4">
        {toasts.map(t => (
          ...
          >
            ...
            <Button variant="ghost" icon onClick={() => dismiss(t.id)} aria-label="Dismiss">×</Button>
          </div>
        ))}
      </div>
