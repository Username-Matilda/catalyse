    ...
    with db_transaction() as conn:
        ...
        cursor = conn.execute(
            ...
            (
                ...
                False, data.time_commitment_hours_per_week, data.urgency,
...
def update_project(
    ...
    with db_transaction() as conn:
        ...
        for field in ["title", "description", "status", "time_commitment_hours_per_week",
                      "urgency", "collaboration_link", "owner_id"]:
            value = getattr(data, field, None)
            if value is not None:
